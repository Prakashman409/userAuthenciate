...
console.log('i am from dishrouter');

dishRouter.route('/')
.get((req,res,next)=>{
    dishes.find({}).populate('comments.author').then((dish)=>{
        res.statusCode=200;
...
.put((req,res,next)=>{
    res.statusCode=403;
    res.send('this operation cannot be done at the present moment');
})
...
.post((req,res,next)=>{
    res.statusCode=403;
    res.send('this operation cannot be done at the current moment');
})
.put((req,res,next)=>{
...
.put((req, res, next) => {
    ...
    .then((dish) => {
        if (dish != null && dish.comments.id(req.params.commentId) != null) {
            ...
            dish.save()
            .then((dish) => {
                dishes.findById(dish_id).populate('comments.author').then((dish)=>{
                    res.statusCode = 200;
                res.setHeader('Content-Type', 'application/json');
                res.json(dish);
                    })
